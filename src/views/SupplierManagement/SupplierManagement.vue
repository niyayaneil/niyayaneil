...
import {
  getSupplierManagementPageApi,
  addSupplierManagementApi,
  editSupplierManagementApi,
  patchSupplierManagementApi,
} from '@/api/SupplierManagement'
import { getOptionsListApi, getreadMsgApi } from '@/api/Public'
import { ElMessage, ElMessageBox } from 'element-plus'
import type { FormRules, FormInstance } from 'element-plus'
import type { BillItemSearchResultItem } from '@/types/SupplierManagement'
onMounted(() => {
  getTableList()
  getOptionsListApi({ supplierTypes: {} }).then((res) => {
    typeList.value = res.data.supplierTypes
  })
  getreadMsgApi({ isValid: '1', pageNum: 1, pageSize: 100 }).then((res: any) => {
    if (res.data.list.length > 0) {
      res.data.list.forEach((item: { code: string; content: string }) => {
        if (item.code === 'suppliers.create') {
...
const getTableList = () => {
  getSupplierManagementPageApi(form.value).then((res) => {
...
// 添加
const add = () => {
  titleDialog.value = 'New Supplier'
  visibleDialog.value = true
}
const edit = (row: BillItemSearchResultItem) => {
  titleDialog.value = 'Edit Supplier'
  visibleDialog.value = true
  nextTick(() => {
...
const handleStatusChange = async (row: { id: number; isValid: string }) => {
  ElMessageBox.confirm('Please confirm this operation', 'Warning', {
    ...
    .then(async () => {
      ...
      await patchSupplierManagementApi({ id: row.id, isValid: row.isValid })
      ElMessage({
        type: 'success',
        message: `Success`,
      })
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      if (titleDialog.value === 'New Supplier') {
        delete formCopy.id
        addSupplierManagementApi(formCopy).then((res) => {
          if (res.data) {
            getTableList()
            closeDialog()
            ElMessage({
        ...
        editSupplierManagementApi(formCopy).then(() => {
          getTableList()
          closeDialog()
          ElMessage({
            type: 'success',
  ...
  <div class="mt-[3px]">
    <div class="p-[24px] bg-white">
      ...
      >
        ...
        <el-table-column prop="nameCn" label="Chinese Name" min-width="138" show-overflow-tooltip />
        <el-table-column prop="nameEn" label="English Name" min-width="138" show-overflow-tooltip />
        <el-table-column prop="code" label="Supplier Code" min-width="138" show-overflow-tooltip />
        <el-table-column prop="type" label="Type" min-width="158" show-overflow-tooltip />
    ...
    >
      ...
      <div>
        ...
        >
          ...
          <el-row :gutter="24">
            <el-col :span="12">
              <el-form-item label="Supplier Code" prop="code">
                <el-input placeholder="Input" v-model.trim="ruleForm.code" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="Type" prop="type">
                <el-select v-model="ruleForm.type" filterable placeholder="Please Select">
                  <el-option
