...
import type { BillItemSearchResultItem } from '@/types/SVMapping'
onMounted(() => {
  onSearch()
  ...
  getreadMsgApi({ isValid: '1', pageNum: 1, pageSize: 100 }).then((res: any) => {
    if (res.data.list.length > 0) {
      res.data.list.forEach((item: { code: string; content: string }) => {
        if (item.code === 'carrierServiceMappings.create') {
...
const getTableList = () => {
  getSVMappingPageApi(form.value).then((res) => {
...
// 添加
const add = () => {
  titleDialog.value = 'New SV Mapping'
  visibleDialog.value = true
}
const edit = (row: BillItemSearchResultItem) => {
  titleDialog.value = 'Edit SV Mapping'
  getOptionsListApi({
    carrierServices: { isValid: '1', carrierCompanyCodeList: [row.carrierCompanyCode] },
  }).then((res: any) => {
...
const handleChangeCarrierServiceCode = () => {
  if (ruleForm.value.carrierCompanyCode) {
    getOptionsListApi({
      carrierServices: {
        isValid: '1',
        carrierCompanyCodeList: [ruleForm.value.carrierCompanyCode],
      },
    }).then((res: any) => {
      if (res.data.carrierServices.length > 0) {
        carrierServiceListValid.value = res.data.carrierServices
        // 检查是否包含当前选中的值
        if (
          ruleForm.value.carrierServiceCode &&
          !carrierServiceListValid.value.find(
            (item) => item.value === ruleForm.value.carrierServiceCode,
          )
        ) {
          ruleForm.value.carrierServiceCode = ''
        }
...
const handleStatusChange = async (row: { id: number; isValid: string }) => {
  ElMessageBox.confirm('Please confirm this operation', 'Warning', {
    ...
    .then(async () => {
      ...
      await patchSVMappingApi({ id: row.id, isValid: row.isValid })
      ElMessage({
        type: 'success',
        message: `Success`,
      })
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      if (titleDialog.value === 'New SV Mapping') {
        delete formCopy.id
        addSVMappingApi(formCopy).then((res) => {
          if (res.data) {
            getTableList()
            closeDialog()
            ElMessage({
        ...
        editSVMappingApi(formCopy).then(() => {
          getTableList()
          closeDialog()
          ElMessage({
            type: 'success',
  ...
  <div class="mt-[3px]">
    <div class="p-[24px] bg-white">
      ...
      >
        ...
        />
        <el-table-column
          prop="carrierServiceCode"
          label="Standard Route Code"
          min-width="168"
          show-overflow-tooltip
        />
        <el-table-column
      ...
      <div>
        ...
        >
          <el-row :gutter="10">
            ...
            <el-col :span="12">
              <el-form-item label="Standard Route Code" prop="carrierServiceCode">
                <el-select
                  v-model.trim="ruleForm.carrierServiceCode"
                  filterable
                  placeholder="Please Select"
                  clearable
                >
