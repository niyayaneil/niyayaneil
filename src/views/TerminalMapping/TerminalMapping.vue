...
import {
  getTerminalMappingPageApi,
  addTerminalMappingApi,
  editTerminalMappingApi,
  patchTerminalMappingApi,
} from '@/api/TerminalMapping'
import { getOptionsListApi, getreadMsgApi } from '@/api/Public'
import { ElMessage, ElMessageBox } from 'element-plus'
import type { FormRules, FormInstance } from 'element-plus'
import type { BillItemSearchResultItem } from '@/types/TerminalMapping'
onMounted(() => {
  onSearch()
  // 找到所有码头
  getOptionsListApi({ globalPortTerminals: {} }).then((res: any) => {
    if (res.data.globalPortTerminals.length > 0) {
      globalPortTerminalList.value = res.data.globalPortTerminals
    }
  })
  // 找到所有港口
  ...
  // 找到所有有效码头
  getOptionsListApi({ globalPortTerminals: { isValid: '1' } }).then((res: any) => {
    if (res.data.globalPortTerminals.length > 0) {
      globalPortTerminalListValid.value = res.data.globalPortTerminals
    }
  })
  getreadMsgApi({ isValid: '1', pageNum: 1, pageSize: 100 }).then((res: any) => {
    if (res.data.list.length > 0) {
      res.data.list.forEach((item: { code: string; content: string }) => {
        if (item.code === 'terminalMappings.create') {
...
const globalPortTerminalList = ref<{ label: string; value: string | number }[]>([])
const globalPortListValid = ref<{ label: string; value: string | number }[]>([])
const globalPortTerminalListValid = ref<{ label: string; value: string | number }[]>([])
...
const tableRef = ref()
const getTableList = () => {
  getTerminalMappingPageApi(form.value).then((res) => {
...
const handleStatusChange = async (row: { id: number; isValid: string }) => {
  ElMessageBox.confirm('Please confirm this operation', 'Warning', {
    ...
    .then(async () => {
      ...
      await patchTerminalMappingApi({ id: row.id, isValid: row.isValid })
      ElMessage({
        type: 'success',
        message: `Success`,
      })
...
// 添加
const add = () => {
  titleDialog.value = 'New Terminal Mapping'
  visibleDialog.value = true
}
const edit = (row: BillItemSearchResultItem) => {
  titleDialog.value = 'Edit Terminal Mapping'
  visibleDialog.value = true
  nextTick(() => {
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      if (titleDialog.value === 'New Terminal Mapping') {
        delete formCopy.id
        addTerminalMappingApi(formCopy).then((res) => {
          if (res.data) {
            getTableList()
            closeDialog()
            ElMessage({
        ...
        editTerminalMappingApi(formCopy).then(() => {
          getTableList()
          closeDialog()
          ElMessage({
            type: 'success',
  ...
  <div class="mt-[3px]">
    <div class="p-[24px] bg-white">
      <el-form
        ...
        label-width="auto"
      >
        <el-form-item label="Terminal" prop="terminalCode">
          <el-select v-model="form.terminalCode" filterable placeholder="Please Select" clearable>
            <el-option
              v-for="item in globalPortTerminalList"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            >
      ...
      <div class="flex">
        <el-button
          ...
          New Terminal Mapping</el-button
        >
      </div>
      <el-table
        :data="tableData"
      ...
      <div>
        ...
        >
          <el-form-item label="English Name" prop="nameEn">
            <el-input v-model="ruleForm.nameEn" placeholder="Input"></el-input>
          </el-form-item>
          <el-form-item label="Chinese Name" prop="nameCn">
            <el-input v-model="ruleForm.nameCn" placeholder="Input"></el-input>
          </el-form-item>
          <el-row :gutter="20">
            <el-col :span="12">
          ...
          <el-form-item label="Standard Port" prop="portCode">
            <el-select
              v-model.trim="ruleForm.portCode"
              filterable
              placeholder="Please Select"
              clearable
            >
          ...
          <el-row :gutter="20">
            <el-col :span="14">
              <el-form-item label="Terminal" prop="terminalCode">
                ...
                >
                  <el-option
                    v-for="item in globalPortTerminalListValid"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  >
