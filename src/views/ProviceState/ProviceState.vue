...
import {
  getProviceStatePageApi,
  addProviceStateApi,
  editProviceStateApi,
  patchProviceStateApi,
} from '@/api/ProviceState'
import { getOptionsListApi, getreadMsgApi } from '@/api/Public'
import { ElMessage, ElMessageBox } from 'element-plus'
import type { FormRules, FormInstance } from 'element-plus'
import type { BillItemSearchResultItem } from '@/types/ProviceState'
onMounted(() => {
  onSearch()
  ...
  getreadMsgApi({ isValid: '1', pageNum: 1, pageSize: 100 }).then((res: any) => {
    if (res.data.list.length > 0) {
      res.data.list.forEach((item: { code: string; content: string }) => {
        if (item.code === 'dictStates.create') {
...
const getTableList = () => {
  getProviceStatePageApi(form.value).then((res) => {
...
// 添加
const add = () => {
  titleDialog.value = 'New Provice/State'
  visibleDialog.value = true
}
const edit = (row: BillItemSearchResultItem) => {
  titleDialog.value = 'Edit Provice/State'
  visibleDialog.value = true
  nextTick(() => {
...
const handleStatusChange = async (row: { id: number; isValid: string }) => {
  ElMessageBox.confirm('Please confirm this operation', 'Warning', {
    ...
    .then(async () => {
      ...
      await patchProviceStateApi({ id: row.id, isValid: row.isValid })
      ElMessage({
        type: 'success',
        message: `Success`,
      })
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      if (titleDialog.value === 'New Provice/State') {
        delete formCopy.id
        addProviceStateApi(formCopy).then((res) => {
          if (res.data) {
            getTableList()
            closeDialog()
            ElMessage({
        ...
        editProviceStateApi(formCopy).then(() => {
          getTableList()
          closeDialog()
          ElMessage({
            type: 'success',
  ...
  <div class="mt-[3px]">
    <div class="p-[24px] bg-white">
      ...
      >
        ...
        <el-table-column prop="id" label="LDS_PK" width="100" show-overflow-tooltip />
        <el-table-column
          prop="countryNameEn"
          label="Country"
          min-width="118"
      ...
      <div>
        ...
        >
          <el-row :gutter="10">
            ...
            <el-col :span="8">
              <el-form-item label="Provice/State Code" prop="numCode">
