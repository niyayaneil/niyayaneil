...
import type { BillItemSearchResultItem } from '@/types/BasePortTT'
onMounted(() => {
  onSearch()
  // 找到所有港口
  ...
  // 找到所有SVC
  getOptionsListApi({ carrierServices: {} }).then((res: any) => {
    if (res.data.carrierServices.length > 0) {
      svcCodeList.value = res.data.carrierServices
    }
  })
  // 找到所有船司
  ...
  // 找到所有Route
  getOptionsListApi({ baseTTRoutes: {} }).then((res: any) => {
    if (res.data.baseTTRoutes.length > 0) {
      routeCodeList.value = res.data.baseTTRoutes
    }
  })
  // 找到所有ETD
  getOptionsListApi({ etds: {} }).then((res: any) => {
    if (res.data.etds.length > 0) {
      etdList.value = res.data.etds
    }
  })
  // 找到所有有效港口
  ...
  getreadMsgApi({ isValid: '1', pageNum: 1, pageSize: 100 }).then((res: any) => {
    if (res.data.list.length > 0) {
      res.data.list.forEach((item: { code: string; content: string }) => {
        if (item.code === 'globalBaseTTs.create') {
...
const onReset = () => {
  ...
  form.value.svcCodeList = []
  form.value.carrierList = []
  form.value.routeCodeList = []
  form.value.etdList = []
...
const svcCodeList = ref<{ label: string; value: string | number }[]>([])
const carrierServiceListValid = ref<{ label: string; value: string | number }[]>([])
const etdList = ref<{ label: string; value: string | number }[]>([])
...
const routeCodeList = ref<{ label: string; value: string | number }[]>([])
// 搜索
const form = ref({
  polCodeList: [],
  podCodeList: [],
...
const getTableList = () => {
  ...
  formCopy.svcCodeList = formCopy.svcCodeList.length > 0 ? formCopy.svcCodeList.join(',') : ''
  formCopy.carrierList = formCopy.carrierList.length > 0 ? formCopy.carrierList.join(',') : ''
  formCopy.routeCodeList = formCopy.routeCodeList.length > 0 ? formCopy.routeCodeList.join(',') : ''
  formCopy.etdList = formCopy.etdList.length > 0 ? formCopy.etdList.join(',') : ''
  getBasePortTTPageApi(formCopy).then((res) => {
...
const handleStatusChange = async (row: { id: number; isValid: string }) => {
  ElMessageBox.confirm('Please confirm this operation', 'Warning', {
    ...
    .then(async () => {
      ...
      await patchBasePortTTApi({ id: row.id, isValid: row.isValid })
      ElMessage({
        type: 'success',
        message: `Success`,
      })
...
// 添加
const add = () => {
  titleDialog.value = 'New Base Port TT'
  visibleDialog.value = true
}
const edit = (row: BillItemSearchResultItem) => {
  titleDialog.value = 'Edit Base Port TT'
  getOptionsListApi({
    carrierServices: { isValid: '1', carrierCompanyCodeList: [row.carrier] },
  }).then((res: any) => {
    if (res.data.carrierServices.length > 0) {
      carrierServiceListValid.value = res.data.carrierServices
    } else {
      carrierServiceListValid.value = []
    }
  })
  visibleDialog.value = true
  nextTick(() => {
...
const handleChangeCarrierServiceCode = () => {
  if (ruleForm.value.carrier) {
    getOptionsListApi({
      carrierServices: { isValid: '1', carrierCompanyCodeList: [ruleForm.value.carrier] },
    }).then((res: any) => {
      if (res.data.carrierServices.length > 0) {
        carrierServiceListValid.value = res.data.carrierServices
        // 检查是否包含当前选中的值
        if (
          ruleForm.value.carrier &&
          !carrierServiceListValid.value.find((item) => item.value === ruleForm.value.svcCode)
        ) {
          ruleForm.value.svcCode = ''
        }
      } else {
        ruleForm.value.svcCode = ''
        carrierServiceListValid.value = []
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      if (titleDialog.value === 'New Base Port TT') {
        delete formCopy.id
        addBasePortTTApi(formCopy).then((res) => {
          if (res.data) {
            getTableList()
            closeDialog()
            ElMessage({
        ...
        editBasePortTTApi(formCopy).then(() => {
          getTableList()
          closeDialog()
          ElMessage({
            type: 'success',
  ...
  <div class="mt-[3px]">
    <div class="p-[24px] bg-white">
      ...
      >
        ...
        <el-form-item label="SVC" prop="svcCodeList">
          <el-select
            v-model="form.svcCodeList"
            filterable
            multiple
            :reserve-keyword="false"
            placeholder="Please Select"
        ...
        <el-form-item label="Carrier" prop="carrierList">
          <el-select
            v-model="form.carrierList"
            filterable
            multiple
            :reserve-keyword="false"
            placeholder="Please Select"
        ...
        <el-form-item label="Route" prop="routeCodeList">
          <el-select
            v-model="form.routeCodeList"
            filterable
            multiple
            :reserve-keyword="false"
            placeholder="Please Select"
        ...
        <el-form-item label="ETD" prop="etdList">
          <el-select
            v-model="form.etdList"
            filterable
            multiple
            :reserve-keyword="false"
            placeholder="Please Select"
      ...
      >
        ...
        <el-table-column prop="polCode" label="POL" min-width="100" show-overflow-tooltip />
        <el-table-column prop="podCode" label="POD" min-width="100" show-overflow-tooltip />
        <el-table-column prop="svcCode" label="SVC" min-width="100" show-overflow-tooltip />
        <el-table-column prop="carrier" label="Carrier" min-width="100" show-overflow-tooltip />
        <el-table-column prop="routeCode" label="Route" min-width="100" show-overflow-tooltip />
        <el-table-column prop="serviceCode" label="Service" min-width="100" show-overflow-tooltip />
        <el-table-column prop="etd" label="ETD" min-width="100" show-overflow-tooltip />
      ...
      <div>
        ...
        >
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="Carrier" prop="carrier">
                <el-select
                  ...
                  filterable
                  placeholder="Please Select"
                  @change="handleChangeCarrierServiceCode"
                  clearable
                >
                  <el-option
            ...
            <el-col :span="12">
              <el-form-item label="SVC" prop="svcCode">
                <el-select
                  v-model.trim="ruleForm.svcCode"
                  filterable
                  placeholder="Please Select"
                  clearable
                >
                  <el-option
                    v-for="item in carrierServiceListValid"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                  >
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="Route" prop="routeCode">
                <el-input v-model="ruleForm.routeCode" placeholder="Input"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="Service" prop="serviceCode">
                <el-input v-model="ruleForm.serviceCode" placeholder="Input"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
