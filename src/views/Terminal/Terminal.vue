...
import type { BillItemSearchResultItem } from '@/types/Terminal'
onMounted(() => {
  onSearch()
  // 找到所有船司
  getOptionsListApi({ globalPortTerminals: {} }).then((res: any) => {
    if (res.data.globalPortTerminals.length > 0) {
      codeList.value = res.data.globalPortTerminals
    }
  })
  // 找到所有港口
  ...
  getreadMsgApi({ isValid: '1', pageNum: 1, pageSize: 100 }).then((res: any) => {
    if (res.data.list.length > 0) {
      res.data.list.forEach((item: { code: string; content: string }) => {
        if (item.code === 'globalTerminals.create') {
...
const tableRef = ref()
const getTableList = () => {
  getTerminalPageApi(form.value).then((res) => {
...
// 添加
const add = () => {
  titleDialog.value = 'New Terminal'
  visibleDialog.value = true
}

const edit = (row: BillItemSearchResultItem) => {
  titleDialog.value = 'Edit Terminal'
  visibleDialog.value = true
  nextTick(() => {
...
const handleStatusChange = async (row: { id: number; isValid: string }) => {
  ElMessageBox.confirm('Please confirm this operation', 'Warning', {
    ...
    .then(async () => {
      ...
      await patchTerminalApi({ id: row.id, isValid: row.isValid })
      ElMessage({
        type: 'success',
        message: `Success`,
      })
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      if (titleDialog.value === 'New Terminal') {
        delete formCopy.id
        addTerminalApi(formCopy).then((res) => {
          if (res.data) {
            getTableList()
            closeDialog()
            ElMessage({
  ...
  <div class="mt-[3px]">
    <div class="p-[24px] bg-white">
      <el-form
        ...
        label-width="auto"
      >
        <el-form-item label="Terminal" prop="code">
      ...
      >
        ...
        <el-table-column prop="code" label="Terminal Code" min-width="138" show-overflow-tooltip />
        <el-table-column prop="nameEn" label="English Name" min-width="188" show-overflow-tooltip />
    ...
    >
      ...
      <div>
        <el-form
          ...
          class="demo-ruleForm"
        >
          <el-form-item label="Terminal Code" prop="code">
            <el-input
              v-model.trim="ruleForm.code"
              placeholder="Port code (5 uppercase letters)"
            ></el-input>
          </el-form-item>
