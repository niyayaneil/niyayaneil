...
const getTableList = () => {
  getCarrierPageApi(form.value).then((res) => {
...
// 添加
const add = () => {
  titleDialog.value = 'New Shipping Line'
  visibleDialog.value = true
}
const edit = (row: BillItemSearchResultItem) => {
  titleDialog.value = 'Edit Shipping Line'
  visibleDialog.value = true
  nextTick(() => {
...
const handleStatusChange = async (row: { id: number; isValid: string }) => {
  ElMessageBox.confirm('Please confirm this operation', 'Warning', {
    ...
    .then(async () => {
      ...
      await patchCarrierApi({ id: row.id, isValid: row.isValid })
      ElMessage({
        type: 'success',
        message: `Success`,
      })
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      if (titleDialog.value === 'New Shipping Line') {
        delete formCopy.id
        addCarrierApi(formCopy).then((res) => {
          if (res.data) {
            getTableList()
            closeDialog()
            ElMessage({
  ...
  <div class="mt-[3px]">
    <div class="p-[24px] bg-white">
      ...
      >
        ...
        <el-table-column prop="name" label="Carrier name" min-width="118" show-overflow-tooltip />
        <el-table-column prop="code" label="Carrier code" min-width="118" show-overflow-tooltip />
        <el-table-column
          prop="cargowise9Code"
          label="Carrier CW9 code"
          min-width="142"
        ...
        <el-table-column prop="webUrl" label="Website URL" min-width="136" show-overflow-tooltip>
          <template #default="{ row }">
        ...
        <el-table-column prop="sort" label="Priority Order" min-width="118" show-overflow-tooltip />
    ...
    >
      ...
      <div>
        <el-form
          ...
          class="demo-ruleForm"
        >
          <el-form-item label="Carrier code" prop="code">
            <el-input placeholder="Enter carrier code" v-model.trim="ruleForm.code" />
          </el-form-item>

          <el-form-item label="Carrier name" prop="name">
            <el-input placeholder="Enter carrier name" v-model="ruleForm.name" />
          </el-form-item>
          <el-row :gutter="24">
            <el-col :span="12">
              <el-form-item label="Carrier CW9 code" prop="cargowise9Code">
                <el-input
                  placeholder="Enter carrier CW9 code"
                  v-model.trim="ruleForm.cargowise9Code"
                />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="LOGO" prop="logoUrl">
                <el-input placeholder="Enter LOGO URL" v-model.trim="ruleForm.logoUrl" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="24">
            <el-col :span="12">
              <el-form-item label="Website URL" prop="webUrl">
                <el-input placeholder="Enter Website URL" v-model.trim="ruleForm.webUrl" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="Priority Order" prop="sort">
                <el-input placeholder="Input" v-model.trim="ruleForm.sort" />
              </el-form-item>
            </el-col>
          </el-row>
