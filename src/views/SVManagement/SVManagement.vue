...
import {
  getSVManagementPageApi,
  addSVManagementApi,
  batchImportSVManagementApi,
  editSVManagementApi,
  patchSVManagementApi,
} from '@/api/SVManagement'
import { getOptionsListApi, getreadMsgApi } from '@/api/Public'
import { ElMessage, ElMessageBox } from 'element-plus'
import type { FormRules, FormInstance } from 'element-plus'
import type { BillItemSearchResultItem } from '@/types/SVManagement'
onMounted(() => {
  onSearch()
  // 找到所有港口
  ...
  // 找到所有SVC
  getOptionsListApi({
    carrierServices: { isValid: '1' },
  }).then((res: any) => {
    if (res.data.carrierServices.length > 0) {
      svcList.value = res.data.carrierServices
    }
  })
  // 找到所有船司
  ...
  getreadMsgApi({ isValid: '1', pageNum: 1, pageSize: 100 }).then((res: any) => {
    if (res.data.list.length > 0) {
      res.data.list.forEach((item: { code: string; content: string }) => {
        if (item.code === 'carrierServices.create') {
...
const onReset = () => {
  form.value.carrierCompanyCodeList = []
  form.value.codeList = []
  form.value.startPortCodeList = []
  form.value.endPortCodeList = []
...
const svcList = ref<{ label: string; value: string | number }[]>([])
// 搜索
const form = ref({
  carrierCompanyCodeList: [],
  codeList: [],
  startPortCodeList: [],
  endPortCodeList: [],
  isValid: '1',
...
const getTableList = () => {
  ...
  formCopy.carrierCompanyCodeList =
    formCopy.carrierCompanyCodeList.length > 0 ? formCopy.carrierCompanyCodeList.join(',') : ''
  formCopy.codeList = formCopy.codeList.length > 0 ? formCopy.codeList.join(',') : ''
  formCopy.startPortCodeList =
    formCopy.startPortCodeList.length > 0 ? formCopy.startPortCodeList.join(',') : ''
  formCopy.endPortCodeList =
    formCopy.endPortCodeList.length > 0 ? formCopy.endPortCodeList.join(',') : ''
  getSVManagementPageApi(formCopy).then((res) => {
...
// 添加
const add = () => {
  titleDialog.value = 'New SV'
  visibleDialog.value = true
}

const edit = (row: BillItemSearchResultItem) => {
  titleDialog.value = 'Edit SV'
  visibleDialog.value = true
  nextTick(() => {
...
const handleStatusChange = async (row: { id: number; isValid: string }) => {
  ElMessageBox.confirm('Please confirm this operation', 'Warning', {
    ...
    .then(async () => {
      ...
      await patchSVManagementApi({ id: row.id, isValid: row.isValid })
      ElMessage({
        type: 'success',
        message: `Success`,
      })
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      if (titleDialog.value === 'New SV') {
        delete formCopy.id
        addSVManagementApi(formCopy).then((res) => {
          if (res.data) {
            getTableList()
            closeDialog()
            ElMessage({
        ...
        editSVManagementApi(formCopy).then(() => {
          getTableList()
          closeDialog()
          ElMessage({
            type: 'success',
...
const uploadAction =
  ...
  '/mgr/carrierServices/import/preview'
const downloadTemplate = () => {
  const url = import.meta.env.VITE_FILE_URL + 'dict-service/en_sv_template.xlsx'
  const a = document.createElement('a')
  a.href = url
  a.download = 'SVManagement.xlsx'
      ...
      return h(
        'div',
        {
          class:
            rowData.error === null
              ? ''
              : rowData.error.errorFields.includes('carrierCompanyCode')
                ? rowData.error.errorType === 'field.empty' ||
                  rowData.error.errorType === 'entity.duplicate'
                  ? 'Hy'
                  : 'Hr'
                : '',
          title: rowData.carrierCompanyCode,
          style: {
            textAlign: 'left',
            width: '200px',
            overflow: 'hidden',
      ...
      return h(
        'div',
        {
          class:
            rowData.error === null
              ? ''
              : rowData.error.errorFields.includes('startPortCode')
                ? rowData.error.errorType === 'field.empty' ||
                  rowData.error.errorType === 'entity.duplicate'
                  ? 'Hy'
                  : 'Hr'
                : '',
          title: rowData.startPortCode,
          style: {
            textAlign: 'left',
            width: '300px',
            overflow: 'hidden',
        ...
        rowData.startPortCode,
      )
    },
  },
  {
    key: 'endPortCode',
    title: 'Port of Destination (POD)',
    dataKey: 'endPortCode',
    width: 300,
    cellRenderer: ({ rowData }) => {
      return h(
        'div',
        {
          class:
            rowData.error === null
              ? ''
              : rowData.error.errorFields.includes('endPortCode')
                ? rowData.error.errorType === 'field.empty' ||
                  rowData.error.errorType === 'entity.duplicate'
                  ? 'Hy'
                  : 'Hr'
                : '',
          title: rowData.endPortCode,
          style: {
            textAlign: 'left',
            width: '300px',
            overflow: 'hidden',
      ...
      return h(
        'div',
        {
          class:
            rowData.error === null
              ? ''
              : rowData.error.errorFields.includes('code')
                ? rowData.error.errorType === 'field.empty' ||
                  rowData.error.errorType === 'entity.duplicate'
                  ? 'Hy'
                  : 'Hr'
...
const submitImport = async () => {
  if (originsList.value.length === 0) {
    ...
    let res = await batchImportSVManagementApi(originsList.value)
    if (res.code === 200) {
      getTableList()
      closeDownDialog()
      ElMessage({
  ...
  <div class="mt-[3px]">
    <div class="p-[24px] bg-white">
      <el-form
        ...
        label-width="auto"
      >
        <el-form-item label="Carrier" prop="carrierCompanyCodeList">
          <el-select
            v-model="form.carrierCompanyCodeList"
            filterable
            multiple
        ...
        <el-form-item label="Service Code" prop="codeList">
          <el-select
        ...
        <el-form-item label="POL" prop="startPortCodeList">
          <el-select
            v-model="form.startPortCodeList"
            filterable
            multiple
            :reserve-keyword="false"
            placeholder="Please Select"
        ...
        <el-form-item label="POD" prop="endPortCodeList">
          <el-select
            v-model="form.endPortCodeList"
            filterable
            multiple
            :reserve-keyword="false"
            placeholder="Please Select"
        ...
        <el-form-item label="Status" prop="isValid">
          <el-select v-model="form.isValid" placeholder="Please Select" clearable>
      ...
      >
        ...
        />
        <el-table-column prop="code" label="Service Code" min-width="118" show-overflow-tooltip />
        <el-table-column prop="startPortCode" label="POL" min-width="136" show-overflow-tooltip />
        <el-table-column prop="endPortCode" label="POD" min-width="136" show-overflow-tooltip />
    ...
    >
      ...
      <div>
        ...
        >
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="Carrier" prop="carrierCompanyCode">
                <el-select
                  v-model="ruleForm.carrierCompanyCode"
                  filterable
                  placeholder="Please Select"
                  clearable
                >
          ...
          <el-form-item label="POL" prop="startPortCode">
            <el-select
              v-model="ruleForm.startPortCode"
              filterable
              placeholder="Please Select"
              clearable
            >
          ...
          <el-form-item label="POD" prop="endPortCode">
            <el-select
              v-model="ruleForm.endPortCode"
              filterable
              placeholder="Please Select"
              clearable
            >
    ...
    >
      <template #header="{ titleId, titleClass }">
        <div class="flex justify-between">
          <div :id="titleId" :class="titleClass">Import SV</div>
          <div class="flex items-center">
            <el-tooltip placement="bottom-end" effect="light" popper-class="mark">
              <template #content>
                <div v-html="htmlContent"></div>
